...
def test_append_row_persists_sorted_csv(tmp_path: Path) -> None:
    ...
    append_row(path, "2024-01-02", 2.05, "manual", second_month=2.01, spread=0.04)
    append_row(path, "2024-01-01", 2.00, "manual", second_month=1.96, spread=0.04)
    append_row(path, "2024-01-02", 2.06, "api", second_month=2.02, spread=0.04)

    frame = pd.read_csv(path)
    assert list(frame["date"]) == ["2024-01-01", "2024-01-02"]
    assert frame.loc[1, "settle"] == 2.06
    assert frame.loc[1, "second_month"] == 2.02
